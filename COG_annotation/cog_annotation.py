...
parser.add_option("-d","--database",dest="whog",help="whog dict")
...
whog = open(options.whog,"rb")
fun = open(options.fun,"rb")

whog_dict = pickle.load(whog)
...
for i in blast:
	...
	if i[0] not in blast_id:
		blast_id.add(i[0])
		if i[1] in whog_dict.keys():
			for i in whog_dict[i[1]]:
				classification += i 
